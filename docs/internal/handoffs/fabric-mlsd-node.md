...
Implement and register a separate Fabric overlay node. It consumes an image, the analysis node's `Array<Vector4>` normalized bottom-left segment output, and optional index-aligned `Array<Float>` confidences. It draws anti-aliased presentation-space lines with configurable color, width, opacity, and minimum confidence. It preserves `FabricImage.textureTransform` on the output and validates malformed or oversized external arrays as recoverable errors. No inference or decoding belongs in this node.

Use a Metal background-copy compute pass and instanced six-vertex line quads instead of per-pixel scanning all 200 segments. Render into a Fabric-managed `.rgba16Float` image at the source storage size, then retain the source transform on the output. The default background is the unmodified source image; an empty segment array should pass the original image through without allocating a target. The overlay can therefore be used independently with compatible typed arrays.

Definition of done: the plug-in bundle compiles its own Metal shader, the node registers with stable typed ports, image orientation and normalized coordinate mapping are explicit, focused shader and bundle checks pass, both Debug and Release builds succeed, package regression tests still pass, and the installed bundle retains its signing/model/license identity. The README documents direct analysis→overlay wiring and the user's scene-authoring boundary.

Non-goals for this milestone: authoring `.fabric` scenes, changing the already-pinned inference model, adding Fabric-wide custom port types, deterministic same-frame export support, or changing MESS/Fabric source.
